const makeJson = (dif) => {
  ...
  const innerFunc = (elmt, index, arr) => {
  ...
  const res = dif.reduce((acc, elmt, index, arr) => {
  ...
  const bracketCount = dif.reduce((acc, el) => {
  ...
  const countCloseBrackets = res.split('').reduce((count, el) => {
    if (el === '}') {
